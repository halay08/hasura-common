...
import {
  TFileDetail,
  TResizeImageQueryParam,
  TUploaderOption,
  TUploadImagesQueryParam,
  TUploadResult,
  UploadType,
} from './types';
...
export class MediaService {
  ...
  private getFileKey(uploadType: string, ext: string) {
    ...
    switch (uploadType) {
      case UploadType.INCIDENT:
        return `uploads/incidents/${fileName}`;
      case UploadType.WORKLOAD:
        return `uploads/workloads/${fileName}`;
  ...
  public async validateImageUploadQueryParams(
    queryParams: TUploadImagesQueryParam
  ): Promise<void> {
    try {
  ...
  public async validateResizeImageQueryParams(
    queryParams: TResizeImageQueryParam
  ): Promise<void> {
    try {
