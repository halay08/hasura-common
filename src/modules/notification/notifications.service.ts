...
export class NotificationService {
  ...
  async sendCustomEmail(to: string[], options: ICustomEmailOptions) {
    ...
    return this.mailerService.send(emailRequestPayload);
  }

  async sendEmail(to: string | string[], options: IEmailOptions) {
    if (!to) {
      return;
    }

    return this.mailerService.sendToTemplate({
      to,
      cc: options.cc,
      bcc: options.bcc,
